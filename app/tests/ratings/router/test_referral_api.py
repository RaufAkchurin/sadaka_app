...
class TestReferralsRatingAPI:
    ...
    async def test_endpoint(self, auth_ac_super, session, dao: DaoSchemas) -> None:
        now = datetime.datetime.now()

        referral_1: Referral = await dao.referral.add(ReferralAddSchema(type=ReferralTypeEnum.JOIN, sharer_id=1))
        ...
        referral_2: Referral = await dao.referral.add(ReferralAddSchema(type=ReferralTypeEnum.JOIN, sharer_id=2))
