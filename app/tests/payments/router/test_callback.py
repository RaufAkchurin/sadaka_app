...
class TestPaymentCallback:
    ...
    async def test_callback_cancelled(self, ac, test_session) -> None:
        ...
        payment_dao = PaymentDAO(session=test_session)
        payments: list[Payment] = await payment_dao.find_all()
        assert len(payments) == 6  # sum of mocked data without new

    @patch("fastapi.Request.client", Address("185.71.76.1", 1234))  # For ip_security checker
    async def test_callback_success(self, ac, test_session, override_get_session_with_commit) -> None:
