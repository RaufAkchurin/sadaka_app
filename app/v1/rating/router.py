...
from app.v1.users.dao import PaymentDAO, ProjectDAO, ReferralDAO, RegionDAO, UserDAO
...
async def get_referral_payments_rating(
    pagination: PaginationParams = Depends(),
    user_data: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_session_with_commit),
):
    ...
    projects_ordered_by_payments = await referral_dao.get_users_sorted_by_income()
    print(projects_ordered_by_payments)
